...
if __name__ == "__main__":
    ...
    exchange_str = input_def("cell BP (string or filename.txt)", "cell500spm.txt")
    ...
    if "speed-module-3" in necessary_items_for_construction:
        ...
        print()
        speed_modules_per_minute = Fraction(
            input_def("How many speed modules are produced per minute?", "49.4")
        )
        construction_time = (
            necessary_items_for_construction["speed-module-3"]
            / speed_modules_per_minute
        )
        print("Construction time = {:.3f} min.".format(float(construction_time)))

        power_consumption = Fraction(
            input_def("What is the power consumption?", "3400")
        )
        print(
            "Number of power plants: {:10.3f}".format(float(power_consumption / 1120))
        )
        temp = dict_bp()
        ...
        debug(
            json.dumps(
                necessary_items_for_construction + temp, indent=4, sort_keys=True
            )
        )
        ...
        for item_name, amount in necessary_items_for_construction.items():
            if item_name not in exclusion:
                necessary_items_for_construction1[item_name] = (
                    necessary_items_for_construction[item_name] / construction_time
                )
        ...
        # creating a link
        additional_parameters = dict_bp(
            {
                "construction-robot": Fraction(65.0),
        ...
        settings = (
            r"data=1-1-19&min=3&belt=express-transport-belt&dm=p3&db=s3&dbc=24&items="
        )
