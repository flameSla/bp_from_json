    ...
    for r in json_all['recipes']:
        if len(r['products'])==1:
            for ingredient in r['ingredients']:
                #ingredient['amount'] /= r['products'][0]['amount']
                #print(r)
                #print(r['name'], ingredient['amount'], r['products'][0]['amount'])
                ingredient['amount'] = Fraction(ingredient['amount'], r['products'][0]['amount'])
        ...
        if not ingredient['name'] in final_ingredients\
           and ingredient['name'] in recipes:
            ...
            for i in recipes[ingredient['name']]:
                res += get_all_ingredients(i, ingredient['amount'] * amount / k, final_ingredients)
        else:
            res = dict_bp({ingredient['name']: amount * ingredient['amount']})
...
if __name__ == "__main__":
    ...
    if 'speed-module-3' in necessary_items_for_construction:
        ...
        for k, v in nuclear_power_plant_1120MW.items():
            ...
            necessary_items_for_construction_nuclear += dict_bp({k: power_consumption / 1120 * v / construction_time})
        debug(json.dumps(necessary_items_for_construction + temp, indent=4, sort_keys=True))

        recipes = get_recipes()
        exclusion = ('speed-module-3', 'productivity-module-3')
        for item_name, amount in necessary_items_for_construction.items():
            if not item_name in exclusion:
