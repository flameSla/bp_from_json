...
def add_train(bp, train_number, locomotives, cars, station_name):
    ...
    for i in range(number_of_rails):
        rail = entity.new_entity('straight-rail',
                                 i*2 - 1,
                                 train_number*4 + 1,
                                 direction=2)
        bp.append_entity(rail)

    train_stop = entity.new_entity('train-stop',
    ...
    for i in range(locomotives):
        locomotive = entity.new_entity('locomotive',
...
def create_wagon(train_car_position, train_number):
    return entity.new_entity('cargo-wagon',
                             7 * train_car_position + 4,
                             train_number*4 + 1,
                             orientation=0.75)
...
def wagon_close_slots(cargo_wagon, slot_count):
    ...
    while slot_count < 40:
        cargo_wagon.set_inventory_filter({"index": slot_count + 1,
                                          "name": "linked-chest"})
        slot_count += 1
...
def append_chests(bp, filtrs, train_car_position, train_number, items):
    ...
    for key, val in filtrs.items():
        inserter = entity.new_entity('stack-inserter',
                                     7 * train_car_position + 1.5 + pos,
                                     train_number*4 - 0.5)
        bp.append_entity(inserter)

        requester = entity.new_entity('logistic-chest-requester',
                                      7 * train_car_position + 1.5 + pos,
                                      train_number*4 - 1.5)
    ...
    for item, amount in contents.items():
        ...
        while amount > 0:
            ...
            if slot_count + slots > 40:
                ...
                amount -= add_items
                cargo_wagon.update_items({item: add_items},
                ...
                cargo_wagon.update_items({item: amount},
                                         name_verification=False)
                amount = 0
                slot_count += slots
...
#############################################
def filtered_trains(bp, contents, train_number, train_car_position,
                    locomotives, cars, station_name):
...
def get_bp(locomotives, cars, contents, station_name, type_of_Train):
    ...
    if type_of_Train == type_of_train.requester_trains:
        ...
        filtered_trains(bp, contents, train_number, train_car_position,
                        locomotives, cars, station_name)
...
if __name__ == "__main__":
    ...
    get_bp(int(locomotives), int(cars), contents,
           station_name, type_of_train.requester_trains)
    get_bp(int(locomotives), int(cars), contents,
           station_name, type_of_train.filtered_trains)
